...
LAUNCH_GUIDES = (
    ...
    },
    {
        ...
        "summary": "For stateless, event-driven code that autoscales to zero and answers over a public HTTPS URL.",
        "result": "A public HTTPS Lambda Function URL (or an API Gateway endpoint)",
        "artifact": "A deployment ZIP, or a container image pushed to Amazon ECR",
        "time": "Moderate — create the function, deploy code, then add a public URL",
        "good_for": ("APIs", "Event-driven services", "WebSocket apps", "Autoscaling workloads"),
        "not_for": "Long-running stateful processes or apps that need persistent local disk — use Fargate or EC2.",
        "prerequisites": (
            "An AWS account with IAM access for Lambda and CloudWatch Logs.",
            "The AWS CLI installed, authenticated, and pointed at the target region.",
            "A stateless function that accepts one event at a time and returns a response.",
        ),
        "steps": (
            {
                "title": "Build a deployment package",
                "body": "Put the code and every runtime dependency at the top level of a ZIP. For Python, install inside a folder with pip install -t before zipping; for Node.js, install node_modules before zipping. The handler file must be at the root, never under a build folder.",
                "checks": ("The ZIP opens with the handler file at the top level.", "The runtime dependencies are inside the ZIP, not installed only on your laptop."),
            },
            {
                "title": "Create the function",
                "body": "Create a Lambda function and choose the runtime that matches your code. Set the handler to file.function, upload the ZIP (or point Lambda at an ECR image), and grant the basic CloudWatch Logs execution role.",
                "commands": (
                    {"label": "Create a function from a ZIP", "text": "aws lambda create-function --function-name <FUNCTION_NAME> --runtime <RUNTIME> --role <EXECUTION_ROLE_ARN> --handler <FILE>.<FUNCTION> --zip-file fileb://<PACKAGE>.zip", "replace": "Replace function name, runtime, IAM role ARN, handler and ZIP path."},
                    {"label": "Update code after a fix", "text": "aws lambda update-function-code --function-name <FUNCTION_NAME> --zip-file fileb://<PACKAGE>.zip", "replace": "Replace function name and ZIP path."},
                ),
            },
            {
                "title": "Configure a public URL",
                "body": "For simple HTTP entry points, enable a Function URL with AuthType NONE. For routing, custom domains or auth in front, use API Gateway and point it at the function. Never expose a function with full admin credentials as its invoker.",
                "commands": (
                    {"label": "Enable a Function URL", "text": "aws lambda create-function-url-config --function-name <FUNCTION_NAME> --auth-type NONE", "replace": "Replace function name."},
                ),
            },
            {
                "title": "Set environment variables and secrets",
                "body": "Store connection strings and API keys in AWS Secrets Manager and reference the secret from the function; a value baked into the ZIP is recoverable by anyone who can read the package. Add non-secret configuration as environment variables.",
                "commands": (
                    {"label": "Set an environment variable", "text": "aws lambda update-function-configuration --function-name <FUNCTION_NAME> --environment VARIABLES_JSON", "replace": "Replace function name and VARIABLES_JSON with a JSON object, e.g. {\"KEY\":\"VALUE\"}."},
                ),
            },
            {
                "title": "Set concurrency limits and watch logs",
                "body": "Reserved concurrency is the cheapest insurance against one function consuming a burst and starving another. CloudWatch Logs shows invocations and errors; CloudWatch alarms can page you on a sustained error rate.",
                "commands": (
                    {"label": "Set reserved concurrency", "text": "aws lambda put-function-concurrency --function-name <FUNCTION_NAME> --reserved-concurrent-executions <N>", "replace": "Replace function name and N."},
                ),
            },
            {
                "title": "Test, attach a domain, and monitor",
                "body": "Invoke the public URL from a private window, including the exact payload your clients will send. If you use API Gateway, map a custom domain there. Confirm cold starts produce a clean 200 and that errors are visible in CloudWatch.",
            },
        ),
        "checklist": (
            "Function URL returns 200 over HTTPS",
            "Handler runs with an empty payload and with the real client payload",
            "Secrets come from Secrets Manager, not the ZIP",
            "Reserved concurrency is set deliberately",
            "CloudWatch Logs show invocations and errors",
        ),
        "sources": (
            {"label": "AWS Lambda Developer Guide", "url": "https://docs.aws.amazon.com/lambda/latest/dg/welcome.html"},
            {"label": "AWS Lambda deployment packages", "url": "https://docs.aws.amazon.com/lambda/latest/dg/deployment-package-v2.html"},
            {"label": "Lambda Function URLs", "url": "https://docs.aws.amazon.com/lambda/latest/dg/lambda-urls.html"},
            {"label": "AWS CLI Lambda reference", "url": "https://docs.aws.amazon.com/cli/latest/reference/lambda/index.html"},
        ),
    },
    {
        ...
        "pace": "More setup",
        "eyebrow": "Serverless functions",
        "title": "Deploy a function app on Azure Functions",
        "summary": "For event-driven, automatic-scaling functions with a public HTTPS endpoint and pay-per-use billing.",
        "result": "A public HTTPS Azure Functions endpoint (default: *.azurewebsites.net)",
        "artifact": "A deployment ZIP from Functions Core Tools, or a container image in Azure Container Registry",
        "time": "Moderate — create the app, publish code, then configure secrets and scale",
        "good_for": ("APIs", "Event-driven services", "Timer/queue-triggered jobs"),
        "not_for": "Long-running stateful processes or apps that need persistent local disk — use Azure Container Apps or App Service with a warm instance.",
        "prerequisites": (
            "A Microsoft account with an Azure subscription.",
            "The Azure CLI installed, authenticated, and set to the target subscription.",
            "Azure Functions Core Tools installed so the local function app can be deployed with func.",
        ),
        "steps": (
            {
                "title": "Create the function app",
                "body": "Create a function app in the runtime and hosting plan you will publish to. The Consumption plan scales to zero and is the cheapest default for a bursty function; a Premium/App Service plan is only needed for long-running or VNet-bound workloads.",
                "commands": (
                    {"label": "Create a Consumption function app", "text": "az functionapp create --resource-group <RESOURCE_GROUP> --consumption-plan-location <REGION> --runtime <RUNTIME> --functions-version <VERSION> --name <APP_NAME> --storage-account <STORAGE_ACCOUNT>", "replace": "Replace resource group, region, runtime, Functions version, app name and storage account."},
                ),
            },
            {
                "title": "Deploy code",
                "body": "Publish from the folder that contains the local function app. Functions Core Tools builds the deployment ZIP and pushes it to the remote app; never upload a folder containing the whole repository or a virtualenv.",
                "commands": (
                    {"label": "Deploy a local function app", "text": "func azure functionapp publish <APP_NAME>", "replace": "Replace the function app name."},
                ),
            },
            {
                "title": "Set environment variables and secrets",
                "body": "Application settings are Azure's environment variables. Put connection strings and API keys in Key Vault and reference the secret from the app setting; never commit them to the ZIP or to a repository.",
                "commands": (
                    {"label": "Set an application setting", "text": "az functionapp config appsettings set --name <APP_NAME> --resource-group <RESOURCE_GROUP> --settings KEY=VALUE", "replace": "Replace app name, resource group, KEY and VALUE."},
                ),
            },
            {
                "title": "Configure triggers, auth and scale",
                "body": "HTTP triggers accept an AuthLevel attribute; a public API uses Anonymous, a private API uses Function or Admin keys. Review the function plan's scale settings and any maximum instances you want to cap.",
            },
            {
                "title": "Test and monitor",
                "body": "Call the function's HTTPS URL from a private window, first without the real client payload and then with it. Application Insights records executions and failures; enable it before launch so a cold-start problem is not a silent mystery.",
            },
        ),
        "checklist": (
            "The function URL returns 200 over HTTPS",
            "Triggers use the right AuthLevel and the endpoint is public only when intended",
            "Secrets come from Key Vault, not the ZIP or repository",
            "Scaling and maximum instances are deliberate",
            "Application Insights captures executions and errors",
        ),
        "sources": (
            {"label": "Azure Functions documentation", "url": "https://learn.microsoft.com/en-us/azure/azure-functions/"},
            {"label": "Azure Functions quickstart (Azure CLI)", "url": "https://learn.microsoft.com/en-us/azure/azure-functions/functions-create-first-function-azure-cli"},
            {"label": "Publish a function with Core Tools", "url": "https://learn.microsoft.com/en-us/azure/azure-functions/functions-run-local"},
            {"label": "Azure Function app settings", "url": "https://learn.microsoft.com/en-us/azure/azure-functions/functions-app-settings"},
        ),
    },
    {
