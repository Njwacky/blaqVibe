...
| Slash commands | `/model`, `/new`, `/help`, `/session`, `/export`, `/approve` |
...
src/
  ...
  tools/            file-read, file-write, file-edit, glob, grep, list-dir, shell,
                    policy (approval rules), custom (template), index (wiring)
```

## Adding a BlaqVibes-specific tool

Copy `src/tools/custom.ts`, implement `execute`, and add it to the array in
`src/tools/index.ts`. Set `requireApproval: true` (or a function of the input)
for anything that changes state. Run `npm run typecheck` afterwards.
