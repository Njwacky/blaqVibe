...
PROD_POSTURE=(
  ...
  # A deploy supplies these two; settings ship dev defaults for both (console
  # mailer, SQLite when DATABASE_URL is unset). python-dotenv never overrides
  # an exported variable, so these win over a contributor's local .env.
...
if env DEBUG=0 DJANGO_LOCAL_DEV=0 DJANGO_PREVIEW=0 E2B_SANDBOX=0 \
        SEED_DEMO=0 RATELIMIT_ENABLE=1 \
        SECRET_KEY='ci-only-production-posture-placeholder-please-never-reuse-07070A' \
