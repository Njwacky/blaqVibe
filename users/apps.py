...
def _provision_after_migrate(sender, **kwargs):
    """Mint the operator account when DJANGO_SUPERADMIN_PASSWORD is set. This
    runs post-migrate (not only via the management command) because operators
    put the password in .env and expect login to work after `migrate`; without
    an on-boot call, "admin / my password" never resolved to a row.
    """
    try:
